...
if (isset($_GET['q']) && !empty($_GET['q'])) {
    ...
    $reponses = $db->executeSelect('entreprise_question,entreprises', '*', "question_id in (select id from questions where questions.libelle LIKE '%" . $q . "%' ORDER BY LIBELLE) AND (entreprise_question.entreprise_id=entreprises.id)", " ORDER BY entreprise_id LIMIT $start, $offset")->fetchAll();
